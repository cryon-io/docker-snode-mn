...
case $PARAM in
    ip)
        ...
        case $VALUE in 
            *:*)
                TEMP=$(sed "s/externalip=.*/externalip=[$VALUE_FOR_SED]:18988/g" "$BASEDIR/../data/snodecoin.conf")
            ;;
            *)
                TEMP=$(sed "s/externalip=.*/externalip=$VALUE_FOR_SED:18988/g" "$BASEDIR/../data/snodecoin.conf")
            ;;
    ...
    ;;
    nodeprivkey)
        TEMP=$(sed "s/masternodeprivkey=.*/masternodeprivkey=$VALUE_FOR_SED/g" "$BASEDIR/../data/snodecoin.conf")
        printf "%s" "$TEMP" > "$BASEDIR/../data/snodecoin.conf"
    ;;
    NODE_VERSION) 
